...
# Description: This script can generate all the bash files (.sh) to run ORCA calculations in SLURM for the Wulver cluster automaticly. 
# Run as: python orca_wulver_submit.py file.com number_of_days number_of_CPUs
...
def write_bash_file(bash_filename, g16_filename, filename):
    ...
    bash_file.write("#SBATCH --account=gor\n")
    bash_file.write("#SBATCH --partition=general\n")
    bash_file.write("#SBATCH --qos=high\n")
...
#    bash_file.write("module load GCC/8.3.0 OpenMPI/3.1.4 ORCA/4.2.1\n")
    bash_file.write("module load easybuild foss/2022b GCC/12.2.0 OpenMPI/4.1.4 ORCA/5.0.4\n")
    bash_file.write("/apps/easybuild/software/ORCA/5.0.4-gompi-2022b/bin/orca "+filename+' > '+g16_filename+'.out\n')
